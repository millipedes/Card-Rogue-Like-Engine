...
SelfView init_self_view(WINDOW * parent, SelfStateRef self_state_ref) {
  ...
  // Art Space
  self_view.art_space = autosize_window(parent, self_state_ref->art_lines, 25, 35);
  box(self_view.art_space, 0, 0);

  // Hand Space
  self_view.selected = 0;
  hand_to_stream(self_view.hand_names_stream, self_state_ref->card_pool);
  self_view.hand_space = autosize_window(parent, self_view.hand_names_stream, 50, 1);

  card_to_stream(self_view.card_text_stream,
      self_state_ref->card_pool.cards[self_view.selected]);
  self_view.card_space = autosize_window(parent, self_view.hand_names_stream, 50, 1);
  self_view.card_space = derwin(parent,
      max_tstream_height(self_view.card_text_stream) + 5, // Name, cost, rarity, border
      max_tstream_len(self_view.card_text_stream) + 4,
      parent_h * 50 / 100,
      max_tstream_len(self_view.hand_names_stream) + 5
  );
