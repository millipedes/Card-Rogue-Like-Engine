...
void add_card_action(Card * card, Action action) {
  if (!card->actions) {
    card->qty_actions = 1;
    card->actions = calloc(1, sizeof(Action));
  } else {
    card->qty_actions++;
    card->actions = realloc(card->actions,
        card->qty_actions * sizeof(Action));
  }
...
Card deep_copy_card(const Card old) {
  ...
  copy.actions = calloc(copy.qty_actions, sizeof(Action));
